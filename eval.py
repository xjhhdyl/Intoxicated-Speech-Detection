...
from model.classification_model import mmWavoiceNet, ECABasicBlock, rescrossSE, interattention, rnnClassfication, \
    mmWavoice
...
if __name__ == '__main__':
    ...
    mmWavoice_model.load_state_dict(torch.load(model_weight_path, map_location=device))
    mmWavoice_model.to(device)
    ...
    with torch.no_grad():
        for i, (data) in tqdm(enumerate(dev_loader), total=len(dev_loader), leave=False, desc="Validation"):
            ...
            outputs = mmWavoice_model(voice_batch_data=voice_inputs, mmwave_batch_data=mmwave_inputs,
                                      batch_label=labels,
                                      is_training=False, )
